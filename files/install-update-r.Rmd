...
## A.1 Installing R {.tabset .tabset-fade .tabset-pills #install-r}
...
- Click __Download R-4.2.2 for Windows__
...
- Click __RTools 4.2__
- Click __Rtools42 installer__
- Open the installer and follow the instructions while keeping the default options
...
Check which Apple PC you have. On the top-left of your screen click the Apple logo, click About This Mac, note what the Processor line says. If it has the word "Intel", you want to follow 2 below, and 1 otherwise.

Go to <https://cran.r-project.org/bin/macosx/> and download one of these: 

1. `R-4.2.2-arm64.pkg (notarized and signed)` if you have an Apple silicon arm64 macOS PC.
2. `R-4.2.2.pkg (notarized and signed)` if you have an Intel-based macOS PC
...
You cannot install R on Chromebooks. Go directly to the [Installing RStudio](#install-rstudio) section for instructions.



## A.2 Installing RStudio {.tabset .tabset-fade .tabset-pills #install-rstudio}

### Windows PC {#install-win}

If you have a previous RStudio installation, please uninstall it before proceeding.

Go to <https://posit.co/download/rstudio-desktop/>, scroll down to Step 2, and follow the download instructions.
...
### Update R {.tabset .tabset-fade .tabset-pills}
...
To update R, you need to remove any old R installations and reinstall R. Follow the instructions from [section A.1](#install-r).
...
You still may want to update packages, though. Check [section C](#update-packages) for that.
...
If an update is available, follow the instructions from [section A.2](#install-rstudio).
